...
**A level is not optional where there is a choice.** `--thinking` alone is enough only where it leaves nothing open: for a family with no levels at all (`gemma-4*`, `glm-4*`, `qwen3*`), and for one with a single level that counts as thinking (`mistral-medium-3.5*`, which thinks at `high` or not at all). For `qwen3.8*`, `deepseek-v4*` and `openai-gpt-oss*` it is refused, because a request that only switches the thinking on is answered at whatever level the endpoint gives it — a default it does not publish, does not put in the response, and may change between two runs that would then read alike. `--no-thinking` never needs a level: off is off. The models that do not think at all take `--no-thinking` — true of them whatever is sent — and refuse the rest.
...
There is no asking for that default, only measuring it, and only where the levels are far enough apart to be told apart: `openai-gpt-oss-120b` answers one small question at 39 tokens (`low`), 66 (`medium`) and 250 (`high`), and at 76 when it is not told — near `medium`, so probably that, which is as much as the numbers say. On `qwen3.8-27b` the same comparison is useless (62 tokens untold against 83 at `low`), so there the default cannot be identified at all. Hence the rule above rather than a table of measured defaults: a level that has to be inferred from token counts is not a level a run can claim to have used.
...
**Workers.** The vitae are independent — each is one prompt built from files that are only read — so `--workers N` hands several of them to the endpoint at once. They share the process's 15 calls a minute rather than multiplying the budget, and they share it by **taking turns**: `helper_functions.wait_for_a_slot` hands out one call every 4.1 seconds — a tenth of a second more than 15 a minute needs, since at exactly four the first and the sixteenth call are a minute apart and a rolling minute counts both — so no two of them are ever made in the same instant. That is the same fifteen a minute, spread instead of bursty, and it is why fifteen workers do not start with a wall of 429s. Everything that writes stays in the main thread, which only collects what the workers return. A model that answers in a minute leaves 14 of the 15 calls unused, and five workers turn that into five. Counting the calls instead — which is what a rate limiter usually does, and what this one did — lets them through in a bunch and then holds every thread at the edge of the window and releases them together: the run starts well, and then falls into exactly the wave the workers were staggered to avoid, since a fast model empties the window before the minute is out. Spacing has no edge to bunch at, and it covers the retries after a refusal for free: a retry goes through the same door and waits for a slot of its own, rather than coming straight back on top of what the endpoint is already refusing. Only one spare vita per worker is ever queued, so a batch that stops early leaves nothing behind but the handful of answers it is already waiting for.

**Stopping.** Ctrl+C stops the submitting and waits for the calls that are already out, since they are paid for whether their answers are read or not — with a slow model that wait can be a quarter of an hour. **Ctrl+C again abandons them and leaves at once**, for when it is the stopping that matters: a run started by mistake, or a machine that has to be shut down. Either way what has been answered is in the checkpoint before the process goes: the second handler writes what is still buffered — up to ten vitae, and answers already paid for — *before* it leaves, since `os._exit` runs no `finally` and flushes no buffer, and the count in its message is the count on disk. The lines are appended with a single `os.write`, which a Python signal handler cannot cut in half (a handler runs between two bytecodes, never inside a system call), so the worst that a write interrupted by the second Ctrl+C can do is write the same lines twice — and a checkpoint is read by vita, not by line. What was abandoned was never a result, so `--resume` asks for it again. The second one has to be rude about it (`os._exit`): a pool's threads are joined when the interpreter exits, so leaving politely would mean waiting for exactly the answer that is being abandoned. For the same reason the batch installs its own `SIGINT` handler — the second Ctrl+C has to be told from the first, and the raised `KeyboardInterrupt` of the default one arrives too late to be of use — and gives the old one back when it is done. A checkpoint line the leaving cut in half is dropped when the file is next read: that vita was never a result, so a resume asks for it again.

**When the endpoint gives up.** With several vitae in flight the endpoint queues them, and a request that never comes back (`APITimeoutError`) says the queue was long, not that this vita is unanswerable — so it is asked for again like a 500 or a 429, up to five times. Nothing is waited out beyond that: the retry queues for a slot like any other call, which is both the pause that keeps it off what the endpoint is already refusing and the reason two calls refused in the same instant do not come back in one — they take their slots one after another instead of counting down the same seconds. An earlier version added a growing, jittered wait of its own here; with the spacing in place that was a wait on top of a wait. A vita it still refuses to answer for is **not a result**: it is left out of the checkpoint, named at the end of the batch, and asked for again by `--resume`, while the rest of the batch carries on. It is never recorded as an empty answer, which would write the vita out unexpanded and never ask about it again. If ten vitae fail the batch stops rather than working through the rest of them.

**How long an answer may take.** `--timeout` (360 seconds by default) is what one request is given before it counts as never answered. It has to be set here because the SDK's own default is ten minutes *and two silent retries of its own*, with half a second between them: a request the endpoint has dropped then takes about half an hour to raise a single `APITimeoutError`, which our five retries turn into six of those — three hours on one vita, with a message every thirty minutes and nothing in between, since the SDK's attempts are logged at `debug` and nothing turns that on. Worse, those hidden retries go out *without a slot*: `wait_for_a_slot` is passed once per call and not once per attempt, so a 429 the SDK retries comes straight back at the endpoint half a second later, in exactly the wave the spacing exists to prevent, and never appears in the log. The client is therefore built with `max_retries=0` (`run_step.connect`) and every retry is made by `call_chat_ai`, where it is printed, spaced and counted into the report. Six minutes is a default read off the runs so far: no run has a median above three minutes a vita, and the worst figures in the older reports — 1831.6 seconds for one deepseek vita, 2009.4 for another — are three ten-minute timeouts stacked, not one long answer. A model that really does need longer wants a larger `--timeout`, not a client that waits quietly.
