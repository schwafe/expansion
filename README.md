...
**When the endpoint gives up.** With several vitae in flight the endpoint queues them, and a request that never comes back (`APITimeoutError`) says the queue was long, not that this vita is unanswerable — so it is waited out like a 500 or a 429, with the wait growing each time (5s, 10s, 15s …) since all three mean the endpoint has more work than it can take. A vita it still refuses to answer for is **not a result**: it is left out of the checkpoint, named at the end of the batch, and asked for again by `--resume`, while the rest of the batch carries on. It is never recorded as an empty answer, which would write the vita out unexpanded and never ask about it again. If ten vitae fail the batch stops rather than working through the rest of them.
